...
us_map <- fiftystater::fifty_states
pre_2010_pop %>% 
  rename(NAME = `...1`) %>% 
...
NORSData %>% janitor::clean_names() %>% 
  ...
  mutate(state = if_else(state == "Washington DC", 
  ...
  summarise(illness_per_100000 = sum(total_illness)/sum(population)/
              n_distinct(year)*100000, 
            outbreaks_per_100000 = sum(total_outbreaks)/sum(population)/
                                         n_distinct(year)*100000
            ) %>% 
  ungroup() -> outbreak_data
make_map <- function(fill_var, legend) {
  fill_var <- enquo(fill_var)
  ggplot(outbreak_data, aes(fill = !!fill_var)) + 
    geom_map(aes(map_id = str_to_lower(state)), map = us_map) + 
...
make_map(illness_per_100000, 'Illness Incidence per \n 100,000 people')
make_map(outbreaks_per_100000, 'Outbreak Incidence per \n 100,000 people')
